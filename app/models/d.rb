    ...
    # assemble the model based on the ftype
      if ds_element.ftype == "File"
        ...
        if ds_element.relation_type == "has_one"
          meta_string += "#{ds_element.relation_type} :#{ds_element.key}, :class_name => '#{gen_class_name(ds_element.relation_ds)}', :foreign_key => :#{ds_element.relation_field}_id, :inverse_of => :#{ds_element.relation_field}, :autosave => true \n"
